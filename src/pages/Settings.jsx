...
function Toggle({ enabled, onChange }) {
  return (
    <button onClick={() => onChange(!enabled)}
      className={`relative rounded-full transition-colors shrink-0 ${enabled ? 'bg-blue-600' : 'bg-[#1e2a45]'}`}
      style={{ width: '36px', height: '20px' }}>
      <span className={`absolute top-0.5 rounded-full bg-white shadow transition-transform ${enabled ? 'translate-x-[17px]' : 'translate-x-0.5'}`}
...
function TrackingScriptTab() {
  ...
  return (
    <div className="space-y-7">
      <div>
        <SectionHeader title="Install tracking script" description="Add this snippet to every page you want to track. Paste it before the closing </body> tag." />
        <div className="relative">
          <pre className="bg-[#080c14] border border-[#1e2a45] rounded-xl p-5 text-xs font-mono text-blue-300 overflow-x-auto leading-relaxed">{snippet}</pre>
          <button onClick={copy}
            className={`absolute top-3 right-3 text-[11px] px-2.5 py-1.5 rounded-lg border transition-all ${
              copied ? 'bg-emerald-950/40 border-emerald-700 text-emerald-400' : 'bg-[#111827] border-[#1e2a45] text-slate-400 hover:border-slate-500 hover:text-white'
      ...
      <div className="bg-[#0b0f1a] border border-[#1e2a45] rounded-xl p-5 space-y-4">
        <div>
          ...
          <div className="flex gap-2">
            <input readOnly value="tvs_prod_a1b2c3d4e5f6"
              className="flex-1 bg-[#111827] border border-[#1e2a45] rounded-lg px-3 py-2 text-xs font-mono text-slate-300 focus:outline-none" />
            <button className="text-xs text-slate-400 bg-[#111827] border border-[#1e2a45] px-3 py-2 rounded-lg hover:border-slate-500 hover:text-white transition-colors whitespace-nowrap">Regenerate</button>
          </div>
        </div>
        <div>
          <label className="block text-xs text-slate-400 mb-1.5">Allowed domains</label>
          <input defaultValue="acme.com, app.acme.com"
      ...
      <div>
        ...
        <div className="flex items-center gap-4 p-4 bg-[#0b0f1a] border border-[#1e2a45] rounded-xl">
          ...
          <button className="ml-auto text-xs text-slate-400 hover:text-white transition-colors">Check again</button>
        </div>
      </div>
    </div>
  );
...
function IntegrationsTab() {
  const [states, setStates] = useState({ hubspot: true, salesforce: false, slack: true, segment: false, zapier: false, webhook: false });
  const toggle = (key) => setStates((s) => ({ ...s, [key]: !s[key] }));
  const integrations = [
    { key: 'hubspot', name: 'HubSpot', description: 'Sync identified companies and contacts directly to HubSpot CRM.', logo: '\uD83D\uDFE0', badge: 'Connected' },
    { key: 'salesforce', name: 'Salesforce', description: 'Push high-intent leads to Salesforce as accounts and contacts.', logo: '\uD83D\uDD35', badge: null },
    { key: 'slack', name: 'Slack', description: 'Get real-time alerts in Slack when high-intent companies visit.', logo: '\uD83D\uDFE3', badge: 'Connected' },
    { key: 'segment', name: 'Segment', description: 'Send TrackVis events to Segment for downstream tools.', logo: '\uD83D\uDFE2', badge: null },
    { key: 'zapier', name: 'Zapier', description: 'Automate workflows by triggering Zaps on visitor events.', logo: '\uD83D\uDFE1', badge: null },
    { key: 'webhook', name: 'Webhooks', description: 'POST visitor data to any custom endpoint in real time.', logo: '\u2699\uFE0F', badge: null },
  ];
  return (
    <div className="space-y-3">
      <SectionHeader title="Integrations" description="Connect TrackVis to your existing stack to automate lead workflows." />
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {integrations.map((int) => (
          <div key={int.key}
            className={`flex items-start gap-4 p-4 rounded-xl border transition-all ${
              states[int.key] ? 'bg-blue-950/10 border-blue-800/30' : 'bg-[#0b0f1a] border-[#1e2a45]'
            }`}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                {int.badge && <span className="text-[10px] text-emerald-400 bg-emerald-950/40 border border-emerald-800/30 px-1.5 py-0.5 rounded-full">{int.badge}</span>}
              </div>
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">{int.description}</p>
              {states[int.key] && <button className="text-[11px] text-blue-400 hover:text-blue-300 mt-2 transition-colors">Configure →</button>}
            </div>
...
function NotificationsTab() {
  ...
  const [triggerStates, setTriggerStates] = useState(Object.fromEntries(triggers.map((t) => [t.key, t.default])));
  return (
    <div className="space-y-7">
      <div>
          ...
          {['off', 'daily', 'weekly'].map((f) => (
            <button key={f} onClick={() => setDigestFreq(f)}
              className={`text-xs px-4 py-2 rounded-lg border font-medium capitalize transition-all ${
                digestFreq === f ? 'bg-blue-600/15 text-blue-400 border-blue-600/25' : 'text-slate-500 border-[#1e2a45] hover:border-slate-500 hover:text-slate-300'
              }`}>{f}</button>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {triggers.map((t) => (
            <div key={t.key} className="flex items-center justify-between p-4 bg-[#0b0f1a] border border-[#1e2a45] rounded-xl">
              ...
              <Toggle enabled={triggerStates[t.key]} onChange={(v) => setTriggerStates((s) => ({ ...s, [t.key]: v }))} />
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="space-y-7">
        ...
        <div className="space-y-2">
          {members.map((m) => (
            <div key={m.email} className="flex items-center gap-4 p-4 bg-[#0b0f1a] border border-[#1e2a45] rounded-xl">
              ...
              <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full border ${ROLE_COLORS[m.role]}`}>{m.role}</span>
              {m.role !== 'Owner' && <button className="text-xs text-slate-600 hover:text-red-400 transition-colors shrink-0">Remove</button>}
            </div>
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="flex gap-2">
          ...
          <select className="bg-[#0b0f1a] border border-[#1e2a45] rounded-lg px-3 py-2 text-xs text-slate-400 focus:outline-none">
            <option>Member</option><option>Admin</option>
          </select>
          <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap">Send invite</button>
        </div>
      </div>
      <div className="p-4 bg-gradient-to-br from-blue-950/30 to-violet-950/20 border border-blue-800/30 rounded-xl">
        <div className="flex items-center justify-between">
          ...
          <button className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-medium transition-colors whitespace-nowrap">Upgrade →</button>
        </div>
      </div>
    </div>
  );
        ...
        {TABS.map((tab, i) => (
          <button key={tab} onClick={() => setActiveTab(i)}
            className={`px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition-all ${
              activeTab === i ? 'border-blue-500 text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-300'
            }`}>{tab}</button>
        ))}
      </div>
