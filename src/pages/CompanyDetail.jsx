  ...
  return (
    <div className="flex items-start gap-3 p-3.5 rounded-xl bg-[#0b0f1a] border border-[#1e2a45] hover:border-slate-600 transition-colors">
      ...
      <div className="flex flex-col gap-1.5 shrink-0">
        <button className="text-[11px] text-blue-400 hover:text-blue-300 bg-blue-500/10 border border-blue-500/20 px-2 py-1 rounded-md transition-colors whitespace-nowrap">LinkedIn →</button>
        <button onClick={() => navigator.clipboard?.writeText(c.email)}
          className="text-[11px] text-slate-500 hover:text-slate-300 bg-[#1a2235] border border-[#2d3f62] px-2 py-1 rounded-md transition-colors">Copy email</button>
      </div>
    </div>
  );
}
    ...
    <div className="relative pl-6">
      ...
      <div className="ml-2 pb-6">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[11px] px-2 py-0.5 rounded-md font-medium text-white/80"
            style={{ background: s.sourceColor + '33', border: `1px solid ${s.sourceColor}55` }}>{s.source}</span>
        ...
        <div className="flex flex-wrap gap-1.5">
          {s.pages.map((p, i) => (
            <span key={i} className="font-mono text-[11px] bg-[#0b0f1a] border border-[#1e2a45] text-slate-400 px-2 py-0.5 rounded-md">{p}</span>
          ))}
        </div>
      </div>
    </div>
...
function InfoRow({ label, value, href }) {
  return (
    <div className="flex items-start justify-between py-2.5 border-b border-[#1a2235] last:border-0">
      ...
      {href ? (
        <a href={href} target="_blank" rel="noreferrer" className="text-xs text-blue-400 hover:text-blue-300 transition-colors text-right">{value}</a>
      ) : (
  ...
  if (!company) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-500 gap-3">
        ...
        <button onClick={() => navigate('/visitors')} className="text-blue-400 text-sm hover:underline">← Back to visitors</button>
      </div>
    );
  }
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${ic.bg} ${ic.text} ${ic.border}`}>{company.intent} intent</span>
            </div>
            <p className="text-sm text-slate-400 mt-2 leading-relaxed max-w-2xl">{company.description}</p>
          ...
          <div className="shrink-0 flex flex-col items-center gap-1">
            <div className="relative w-16 h-16">
              ...
              <span className={`absolute inset-0 flex items-center justify-center text-lg font-bold ${scoreColor(company.score)}`}>{company.score}</span>
            </div>
        ...
        <div className="xl:col-span-3 bg-[#0f1623] border border-[#1e2a45] rounded-xl overflow-hidden">
          ...
          <div className="p-5">
            <div className="relative">
              ...
              {company.sessions.map((s) => <SessionCard key={s.id} s={s} />)}
            </div>
          </div>
        </div>
        ...
        <div className="xl:col-span-2 space-y-4">
          <div className="bg-[#0f1623] border border-[#1e2a45] rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-[#1e2a45] flex items-center justify-between">
              ...
              <button className="text-xs text-blue-400 hover:text-blue-300 border border-blue-500/20 bg-blue-500/10 px-2.5 py-1 rounded-lg transition-colors">Find more</button>
            </div>
            <div className="p-4 space-y-3">
              {company.contacts.map((c) => <ContactCard key={c.id} c={c} />)}
            </div>
          </div>
          ...
          <div className="bg-[#0f1623] border border-[#1e2a45] rounded-xl overflow-hidden">
            ...
            <div className="px-5 pb-4 pt-2">
              <button className="w-full text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg font-medium transition-colors">Save to CRM</button>
            </div>
          </div>
        </div>
      </div>
