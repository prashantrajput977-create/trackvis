...
function IntentBadge({ intent }) {
  ...
  return <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full border ${c.bg} ${c.text} ${c.border}`}>{intent}</span>;
}
...
function ScoreDot({ score }) {
  ...
  return (
    <div className="relative w-10 h-10 shrink-0">
      ...
      <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-slate-300">{score}</span>
    </div>
  );
}
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      list = list.filter((c) =>
        ...
        c.industry.toLowerCase().includes(q) || c.location.toLowerCase().includes(q));
    }
    if (intentFilter !== 'All') list = list.filter((c) => c.intent === intentFilter);
    list.sort((a, b) => {
      let av = a[sortField], bv = b[sortField];
      if (sortField === 'lastSeen' || sortField === 'firstSeen') { av = new Date(av); bv = new Date(bv); }
    ...
    return (
      <th onClick={() => toggleSort(field)}
        ...
        <span className="flex items-center gap-1">
          {children}
          <span className="text-slate-700">{active ? (sortDir === 'asc' ? ' ↑' : ' ↓') : ' ↕'}</span>
        </span>
      </th>
    );
  }
    ...
    <div className="p-6 space-y-5 max-w-[1400px]">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Identified companies visiting your site</p>
        </div>
        <button className="flex items-center gap-1.5 text-xs text-slate-400 bg-[#0f1623] border border-[#1e2a45] px-3 py-1.5 rounded-lg hover:border-slate-500 hover:text-white transition-colors">
          <svg viewBox="0 0 16 16" className="w-3.5 h-3.5 fill-current"><path d="M8 11L4 7h3V1h2v6h3L8 11zM2 13h12v2H2v-2z" /></svg>
          Export CSV
        </button>
      </div>
            ...
            {INTENT_OPTIONS.map((opt) => (
              <button key={opt} onClick={() => setIntentFilter(opt)}
                className={`text-xs px-2.5 py-1 rounded-lg border transition-all ${
                  intentFilter === opt ? 'bg-blue-600/15 text-blue-400 border-blue-600/25' : 'text-slate-500 border-transparent hover:border-[#1e2a45] hover:text-slate-300'
                }`}>{opt}</button>
            ))}
          </div>
          <span className="text-xs text-slate-600 font-mono">{filtered.length} of {companies.length}</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[900px]">
            ...
            <tbody className="divide-y divide-[#1a2235]">
              {filtered.length === 0 ? (
                <tr><td colSpan={9} className="px-5 py-16 text-center text-slate-600 text-sm">No companies match your filter.</td></tr>
              ) : filtered.map((c) => {
                const lastSession = c.sessions[0];
                return (
                  <tr key={c.id} onClick={() => navigate(`/visitors/${c.id}`)}
                    ...
                    <td className="px-4 py-3.5"><span className="text-xs text-slate-400 bg-[#1a2235] px-2 py-0.5 rounded-md">{c.industry}</span></td>
                    <td className="px-4 py-3.5 max-w-[120px]"><span className="text-xs font-mono text-slate-500 truncate block">{lastSession?.pages?.[0] ?? '—'}</span></td>
                    <td className="px-4 py-3.5"><span className="text-xs text-slate-400 font-mono">{fmt(c.avgDuration)}</span></td>
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-0.5">
                          {Array.from({ length: Math.min(c.totalVisits, 7) }).map((_, i) => <div key={i} className="w-1 h-3 rounded-sm bg-blue-600/60" />)}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 whitespace-nowrap"><span className="text-xs text-slate-500 font-mono">{timeAgo(c.lastSeen)}</span></td>
                    <td className="px-4 py-3.5"><ScoreDot score={c.score} /></td>
                    <td className="px-4 py-3.5"><IntentBadge intent={c.intent} /></td>
                  </tr>
                );
              })}
            </tbody>
